...
export function PortfolioCategoryTable({ variant, categories, isLoading, onEditClick, onDeleteClick, onRestoreClick, onForceDeleteClick }: PortfolioCategoryTableProps) {
  ...
  return (
    <Table>
      ...
      <TableBody>
        {categories.map((cat) => (
          <motion.tr key={cat.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-muted/50">
            <TableCell className="font-medium">{cat.name}</TableCell>
            <TableCell>{cat.description || "-"}</TableCell>
            {variant === "trashed" && <TableCell className="text-muted-foreground text-sm">{cat.deletedAt ? new Date(cat.deletedAt).toLocaleDateString("id-ID") : "-"}</TableCell>}
