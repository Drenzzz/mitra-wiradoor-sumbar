...
export function ArticleCategoryTable({ variant, categories, isLoading, error, onRefresh, onEditClick }: ArticleCategoryTableProps) {
  ...
  const handleRestore = (categoryId: string) => {
    handleAction(fetch(`/api/article-categories/${categoryId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "restore" }) }), {
      loading: "Memulihkan...",
      success: "Kategori dipulihkan.",
      error: "Gagal memulihkan.",
    });
  };
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {variant === "active" ? (
                      <>
                        ...
                        <DropdownMenuItem className="text-red-500" onSelect={() => handleSoftDelete(category.id)}>
                      ...
                      <>
                        ...
                        <DropdownMenuItem className="text-red-500" onSelect={() => handlePermanentDelete(category.id)}>
                          <Trash2 className="mr-2 h-4 w-4" />
                          Hapus Permanen
                        </DropdownMenuItem>
                      </>
