...
export function CategoryTable({ variant, categories, isLoading, error, searchTerm, onRefresh, onEditClick, selectedRowKeys, setSelectedRowKeys, onDeleteClick, onRestoreClick, onForceDeleteClick }: CategoryTableProps) {
      ...
      <TableBody>
        {categories.length > 0 ? (
          categories.map((category) => (
            <motion.tr key={category.id} variants={itemVariants} layout className="hover:bg-muted/50">
              <TableCell className="py-4">
                <Checkbox checked={selectedRowKeys.includes(category.id)} onCheckedChange={(checked) => handleRowSelect(category.id, Boolean(checked))} />
              </TableCell>
              ...
              {variant === "trashed" && <TableCell className="text-muted-foreground py-4">{category.deletedAt ? new Date(category.deletedAt).toLocaleDateString("id-ID") : "–"}</TableCell>}
                  ...
                  <DropdownMenuContent align="end" className="bg-background/80 backdrop-blur-sm">
                    ...
                    {variant === "active" ? (
                      <>
                        ...
                        <DropdownMenuItem className="text-red-600 focus:text-red-600 focus:bg-red-500/10" onSelect={() => onDeleteClick(category)}>
                      ...
                      <>
                        ...
                        <DropdownMenuItem className="text-red-600 focus:text-red-600 focus:bg-red-500/10" onSelect={() => onForceDeleteClick(category)}>
                          <Trash2 className="mr-2 h-4 w-4" />
                          Hapus Permanen
                        </DropdownMenuItem>
                      </>
          ...
          <TableRow>
            <TableCell colSpan={variant === "active" ? 3 : 4} className="h-24 text-center text-muted-foreground">
              {searchTerm ? `Tidak ditemukan kategori dengan nama "${searchTerm}".` : variant === "active" ? "Belum ada kategori." : "Tidak ada kategori di sampah."}
            </TableCell>
          </TableRow>
        )}
      </TableBody>
