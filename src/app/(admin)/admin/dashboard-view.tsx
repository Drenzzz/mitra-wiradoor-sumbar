...
export function DashboardStats({ counts, revenue }: DashboardStatsProps) {
  const statCards = [
    {
...
export function DashboardSalesTrend({ data }: DashboardSalesTrendProps) {
  return (
...
export function DashboardOrderStatus({ data }: DashboardOrderStatusProps) {
  return (
      ...
      <CardContent>
        <div className="h-[300px] w-full flex items-center justify-center">
          {data.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="count" nameKey="status">
                  {data.map((entry) => (
