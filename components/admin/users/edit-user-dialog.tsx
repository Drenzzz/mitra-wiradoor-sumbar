...
export function EditUserDialog({ user, isOpen, onClose, onSuccess }: EditUserDialogProps) {
  ...
  // Isi form dengan data user saat dialog dibuka
  useEffect(() => {
    if (user && isOpen) {
      form.reset({
        name: user.name || '',
  ...
  const onSubmit = async (values: UserUpdateFormValues) => {
    ...
    // Saring data: jangan kirim password jika string kosong
    ...
    toast.promise(
      fetch(`/api/users/${user.id}`, { // Panggil API PATCH
        method: 'PATCH',
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Perbarui detail untuk akun {user?.name || 'pengguna'}.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
