...
export default function CategoryManagementPage() {
  const { categories, totals, isLoading, error, searchTerm, setSearchTerm, sortBy, setSortBy, activeTab, setActiveTab, currentPage, setCurrentPage, rowsPerPage, fetchCategories } = useCategoryManagement();
  ...
  const handleSingleAction = (item: Category, action: "delete" | "restore" | "forceDelete") => {
    const configMap = {
      delete: {
  ...
  const renderCategoryCard = (variant: "active" | "trashed") => {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>{isTrash ? "Kategori yang telah dihapus." : "Daftar semua kategori produk yang tersedia."}</CardDescription>
          <div className="flex flex-col md:flex-row items-center gap-2 pt-4">
            ...
            <div className="relative w-full md:grow">
              <Input placeholder="Cari nama kategori..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full" />
            </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <Tabs defaultValue="active" onValueChange={(v) => setActiveTab(v as "active" | "trashed")} className="w-full">
        <TabsList>
