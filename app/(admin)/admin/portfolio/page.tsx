...
export default function PortfolioManagementPage() {
  const {
    ...
    rowsPerPage: itemRows,
    fetchItems,
    fetchCategories: refreshFilters,
  ...
  const confirmDeleteItem = async () => {
    ...
    toast.promise(fetch(`/api/portfolio/${selectedItem.id}`, { method: "DELETE" }), {
      loading: "Menghapus...",
      success: () => {
        fetchItems();
  ...
  const handleCatAction = async () => {
    ...
    if (catActionType === "restore") {
      method = "PATCH";
      body = JSON.stringify({ action: "restore" });
    }

    toast.promise(fetch(url, { method, headers: body ? { "Content-Type": "application/json" } : undefined, body }), {
      loading: "Memproses...",
      success: () => {
        refreshCategories();
        refreshFilters();
      ...
      <Tabs defaultValue="projects" className="w-full">
          ...
          <Card>
            ...
            <CardFooter>
              <div className="flex items-center space-x-2 ml-auto">
                <Button variant="outline" size="sm" onClick={() => setItemPage((p) => p - 1)} disabled={itemPage <= 1}>
                  Prev
                </Button>
                <Button variant="outline" size="sm" onClick={() => setItemPage((p) => p + 1)} disabled={itemPage >= Math.ceil(itemCount / itemRows)}>
                  Next
                </Button>
              </div>
            </CardFooter>
        ...
        <TabsContent value="categories">
          ...
          <Card>
            ...
            <CardFooter>
              <div className="flex items-center space-x-2 ml-auto">
                <Button variant="outline" size="sm" onClick={() => setCatPage((p) => p - 1)} disabled={catPage <= 1}>
                  Prev
                </Button>
                <Button variant="outline" size="sm" onClick={() => setCatPage((p) => p + 1)} disabled={catPage >= Math.ceil((catTab === "active" ? catCount.active : catCount.trashed) / catRows)}>
                  Next
                </Button>
              </div>
            </CardFooter>
